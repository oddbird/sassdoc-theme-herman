 ...
 * block.
 * For locally-hosted fonts, expects font name, followed by curly-bracketed list
 * of formats and optional parenthetical list of variants (defaults to all
 * variants).
 * NOTE: Locally-hosted fonts require global `fontpath` Herman setting.
 */
module.exports = env => {
  const valid_formats = ['ttf', 'otf', 'woff', 'woff2', 'svg', 'svgz', 'eot'];
  // Matches first word, optionally followed by text within `()`
  // and text within `{}`
  const RE = /([\w-'"]+)?\s*(?:\{(.*)\})?\s*(?:\(([^\]]*)\))?\s*(?:\{(.*)\})?/;
  return {
    name: 'font',
