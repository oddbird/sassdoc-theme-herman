...
const rst = require('@frantic1048/est').default;
...
module.exports = ctx => {
  ...
  /**
   * Load `extraDocs` files, and parse contents as Markdown or reStructuredText.
   */
  if (ctx.herman.extraDocs && ctx.herman.extraDocs.length) {
    ctx.extraDocs = [];
    for (const doc of ctx.herman.extraDocs) {
      let parsed, filepath, filename, name, format;
      if (typeof doc === 'string') {
        filepath = path.resolve(ctx.dir, doc);
        parsed = path.parse(filepath);
        name = filename = parsed.name;
        format = parsed.ext === '.rst' ? 'rst' : 'md';
      } else {
