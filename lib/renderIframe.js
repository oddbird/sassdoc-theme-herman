...
module.exports = (env, item, type) => {
  ...
  if (shouldRender) {
    ...
    if (
      ...
      env.herman &&
      (env.herman.customPropertiesCSS || env.herman.customCSS) &&
      !env.customCSSProps
    ) {
      let promise;
      /* istanbul ignore else */
      if (env.herman.customPropertiesCSS) {
        promise = fs
          .readFile(env.herman.customPropertiesCSS, 'utf-8')
          .catch((err) => {
            env.logger.warn(
              `Error reading "herman.customPropertiesCSS" file: ${err.code}\n`,
            );
            env.customCSSProps = '[data-herman-preview] {}';
          });
      } else if (env.herman.customCSS) {
        promise = fs.readFile(env.herman.customCSS, 'utf-8').catch((err) => {
          env.logger.warn(
