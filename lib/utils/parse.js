...
const getFormats = (fmts, src_path) => {
  const validFormats = {};
  let formats = fmts;
  ...
  for (const format of formats) {
    if (fontUtils.isValidFormat(format)) {
      ...
      if (src && !src.startsWith('data:')) {
        dest = `assets/fonts/${unixify(src)}.${format}`;
        src = path.normalize(`${src}.${format}`);
      }
      ...
      if (isValid) {
        ...
        for (const variantBit of variant.split(' ')) {
          if (fontUtils.constants.weight.has(variantBit)) {
            weight = fontUtils.mapWeight(variantBit);
          }
        ...
        const variantData =
          data[variantKeys[0]] || data[variantKeys[1]] || data[variantKeys[2]];
        if (variantData) {
          // Local or embedded font
          isLocal = true;
          // Get base path to variant font files
          const src_path =
            typeof variantData === 'string' ? variantData : variantData.path;
          if (data.formats) {
            formats = getFormats(data.formats, src_path);
          }
          if (typeof variantData !== 'string') {
            // Set variant-specific `local` setting
            if (variantData.local) {
              local =
                typeof variantData.local === 'string'
                  ? [variantData.local]
                  : variantData.local;
            }
            // Set variant-specific `svgid` setting
            if (variantData.svgid) {
              svgid = variantData.svgid;
            }
  ...
  customCSS: (file, enc, env) => {
    ...
    for (const match of matches) {
      ...
      if (!doNotRewrite.test(src_path)) {
        ...
        const abs_path = path.posix.resolve(
          unixify(src_dir),
          unixify(src_path),
        );
        let new_path;
        if (env.localFonts && env.localFonts.includes(path.resolve(abs_path))) {
